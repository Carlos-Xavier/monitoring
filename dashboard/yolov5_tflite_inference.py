...
class yolov5_tflite:
    ...
    def __init__(self,weights=os.path.join(CURRENT_DIR, 'yolov5s-fp16.tflite'), image_size = 416,conf_thres=0.4,iou_thres=0.45):
        ...
        with open(os.path.join(CURRENT_DIR, 'class_names.txt')) as f:
    ...
    def detect(self, image, SHOW_IMAGES):
        ...
        for i,r in enumerate(result_boxes):
                if result_class_names[i] == 'cat':
                        if result_scores[i] >= 0.6:
                                return True
        return False